"""
Generator script to compile publication-ready Jupyter Notebooks with
rich markdown formatting, analytical formulas, and matplotlib/seaborn plots.
"""

import json
from pathlib import Path

def create_master_pipeline_notebook(output_path: Path):
    cells = [
        {
            "cell_type": "markdown",
            "metadata": {},
        ...
        },
        {
            ...
            "source": [
                ...
                "\n",
                "# Directory Configuration (Handles execution from root, subdirectories, notebooks/, or fresh clones)\n",
                "candidates_raw = [\n",
                ...
                "    # Synthetic dataset fallback for fresh clones / headless testing\n",
                "    Path.cwd() / 'data' / 'dummy_test_city',\n",
                "    Path.cwd().parent / 'data' / 'dummy_test_city',\n",
                "    Path('../data/dummy_test_city'),\n",
                "    Path('data/dummy_test_city'),\n",
                "]\n",
                "RAW_DATA_DIR = next((c for c in candidates_raw if (c / 'location.csv').exists()), None)\n",
                "if RAW_DATA_DIR is None:\n",
                "    try:\n",
                "        from src.generate_dummy_data import generate_dummy_cchain_data\n",
                "        RAW_DATA_DIR = generate_dummy_cchain_data()\n",
                "    except Exception:\n",
                "        RAW_DATA_DIR = Path('data/dummy_test_city')\n",
                "\n",
                ...
                "\n",
                "# Detect Pilot City dynamically (Supports CDO production data and synthetic fallback city)\n",
                ...
                "    PILOT_CITY_NAME = str(df_loc_init['adm3_en'].iloc[0]) if 'adm3_en' in df_loc_init.columns else 'Synthetic Test City'\n",
    ...
    print(f"[SUCCESS] Wrote master pipeline notebook to: {output_path}")

def create_synthetic_validation_notebook(output_path: Path):
    cells = [
        {
            "cell_type": "markdown",
            "metadata": {},
